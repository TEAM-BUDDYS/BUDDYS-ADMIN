...
```bash
rg --files -uu -g '!.git' -g '!node_modules' -g '!dist'
cat package.json
```
...
- 스타일과 token: `docs/architecture/styling.md`
- 코딩 규칙: `docs/conventions/conventions.md`
- Git과 Jira: `docs/conventions/git.md`
- 작업 절차: `docs/workflows/*`

4. 변경 대상과 가까운 기존 코드를 검색합니다. 문서에 예정된 구조와 실제 설치된 라이브러리를 구분하고, 문서만 보고 새 패턴을 만들지 않습니다.
...
- 라우터, API 클라이언트, 상태 또는 스타일링 라이브러리가 설치되어 있다고 가정하지 않습니다.
- 실제 요구사항이 확정되지 않은 계층이나 추상화를 미리 생성하지 않습니다.
